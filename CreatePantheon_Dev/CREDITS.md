...
- **AI Power tier (v19):** vanilla AI does not replan districts for pantheon thresholds, so `ScorePower` now ranks direct tile-yield Powers highest — `FERTILITY_RITES` / `GOD_OF_CRAFTSMEN` / `SHENNONG` / `GGV` at **2.6** (`YIELD_POWER_BONUS` +1.6), above Divine Spark (2.2). `INITIATION_RITES` kept at +0.7 (no longer tied to Fertility). City Patron / Monument / Wine remain lower (placement-heavy).
- **modinfo criteria (v23):** `ModInUse inverse="1"` is not a valid Civ6 criterion child attribute (Modding.log: `Unhandled criterion` / `malformed`). Split into `CreatePantheon_GS` + `CreatePantheon_NoDarkAge` with `inverse` on the Criteria element. Fixes MP lobby “broken mod” after host updates when criteria check fails.
- **MP pantheon confirm (v22):** human chooser used `GameEvents.CP_FoundPantheon.Call`, which only runs on the calling machine — guests appeared to confirm each turn while the host never saw their pantheon. Confirm now uses `UI.RequestPlayerOperation(..., EXECUTE_SCRIPT, OnStart=CP_FoundPantheon)` (same pattern as Haikesi planter/relic). Handler accepts `BeliefIndex` from the param table; idempotent if already founded.
- **Religion chooser leak (v21):** `BELIEF_CLASS_CP_COMBO` `MaxInReligion` **1→0** (same as `CP_DISABLED`). ReligionScreen with no class filter lists every non-`PANTHEON` class — combo rows like “Goddess of Beauty + Sacred Light” were showing next to Crusade. Founding still uses Lua `GameEvents.CP_FoundPantheon`; engine/AI cannot pick `MaxInReligion=0` classes.
- **Builder lag trim (v20):** stop cartesian-product Requirement/Modifier bloat that freezes builder selection. `CP_Requirements.sql`: `RS/REQ_PLOT_HAS_<Improvement>` only for ~12 pantheon-used improvements (not full `Improvements`); drop unused `_WITH_RESOURCE`. `CP_Pantheons.sql`: district REQs only for `District_GreatPersonPoints` + Holy Site; remove unused DS per-district RS; **delete** legacy God-of-Miracles SQL grant Modifiers (Godhood × District × Building) — Lua `CP_Miracles` already owns grants. Load-time `DELETE` cleans leftover miracle rows. **UI fix lives in Haikesi** and activates only when Create Pantheon (Dev or Workshop) is detected; otherwise Haikesi keeps vanilla UnitPanel yield previews.
- **PENDING VERIFY (效果待测待查):** Divine Spark v16 city GPP must be tested in a fresh game, including Russia/Lavra overlap and save/load; v17/v18 Miracle grants need Lavra/Harbor completion + tech-gated Encampment paths; housing/wine load rescan rewritten but still needs in-game confirm. Do not treat GPP/housing/amenity as confirmed fixed until retested. v19: confirm AI `[CP_AI] powerScores` after reload prefers yield Powers over City Patron when terrain godhood fits.
