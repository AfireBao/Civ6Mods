-- Autogen from Base UnitPanel.lua GetUnitActionsTable
-- Builder lag fix (Terra Mirabilis / Builder Lag Fix):
-- 1) First CanStartOperation(..., true) → IMPROVEMENTS list for buttons.
-- 2) Per-improvement CanStartOperation(..., false, false) → can-build filter only
--    (no Requirement reload / yield preview — that was the freeze).
-- Tradeoff: tooltips may omit projected yields; illegal builds stay disabled/grey.
	...
	if isHasMovesLeft then
		...
		for operationRow in GameInfo.UnitOperations() do
			...
			if IsBuildingImprovement(actionHash) then
				...
				-- Need results here: UnitOperationResults.IMPROVEMENTS list for the panel buttons.
				...
				if (bCanStart and tResults ~= nil) then
					if (tResults[UnitOperationResults.IMPROVEMENTS] ~= nil and #tResults[UnitOperationResults.IMPROVEMENTS] ~= 0) then
						...
						for i, eImprovement in ipairs(tImprovements) do
							...
							-- Validity only: false,false skips Requirement reload + yield preview (lag),
							-- but still returns whether this improvement can be built on the plot.
							local bOk, tDetail = UnitManager.CanStartOperation(
								pUnit, actionHash, nil, tParameters, false, false);
							local isDisabled		= not bOk;
							local toolTipString		= Locale.Lookup(operationRow.Description) .. ": " .. Locale.Lookup(improvement.Name);

							if tDetail ~= nil then
								if (tDetail[UnitOperationResults.ADDITIONAL_DESCRIPTION] ~= nil) then
									for _,v in ipairs(tDetail[UnitOperationResults.ADDITIONAL_DESCRIPTION]) do
										toolTipString = toolTipString .. "[NEWLINE]" .. Locale.Lookup(v);
									end
								end
								if isDisabled then
									if (tDetail[UnitOperationResults.FAILURE_REASONS] ~= nil) then
										for _,v in ipairs(tDetail[UnitOperationResults.FAILURE_REASONS]) do
											toolTipString = toolTipString .. "[NEWLINE]" .. "[COLOR:Red]" .. Locale.Lookup(v) .. "[ENDCOLOR]";
										end
									end
								end
