...
| 回响施放 | `NW_AI_ECHO_*`（8） |
| 混乱干扰 | `NW_AI_BARBARIAN_INVASION`、`NW_AI_LIGHTNING_STORM` |
...
选中后从**下一游戏回合**起连续 10 回合，每回合按存活主要文明数 `GameRandomEvents.ApplyEvent` 同等场次官方风暴；落点由引擎决定。与南蛮入侵共享每轮混乱干扰互斥（落地由候选池互斥，不写进大模型提示词）。实现：`Haikesi_LightningStorm_GamePlay.lua`。
