...
| 回响施放 | `NW_AI_ECHO_*`（8） |
| 混乱干扰 | `NW_AI_BARBARIAN_INVASION`（可扩展） |
...
3. **不可重复海克斯会选满**：`IsRepeatable=0` 的卡每个 AI 终身最多各 1 次；南蛮入侵 `IsRepeatable=1` 可重复，但每轮全场至多 1 个 AI。
4. **互斥逻辑已修**：仅剩「南蛮入侵」可选的 AI 会优先占用本轮唯一名额。
