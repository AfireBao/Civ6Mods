...
- 新增林肯专属「解放黑奴」（权重 300）：工业区/水渠 +100% 生产力；工业区在官方 +2 基础上额外 +1 宜居度，合计 +3，并保留 +3 忠诚度；种植园在原有 -2 外额外 -3，合计 -5 忠诚度。额外忠诚度惩罚由海克斯通过 `COLLECTION_PLAYER_IMPROVEMENTS` 遍历持有者的种植园并逐座挂载，避免改良上下文无法响应运行时玩家 Property。工人可直接收获 `Improvement_ValidResources` 中登记为可建造种植园的奢侈品或加成资源，无需先建造种植园；通过隐藏代理资源复用原生收获结算，获得食物和等量金币、移除已有改良、消耗 1 次劳动力并生成当前最先进近战单位，随后直接启用官方 `ABILITY_LINCOLN_MELEE_UNITS`，自动跟随原版/BBG 的赠兵属性差异。
