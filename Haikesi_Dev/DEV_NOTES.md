...
| 和平互利 | `NW_AI_CELESTIAL_EMPIRE`（天朝上国；可扩展） |
...
## NW_AI_CELESTIAL_EMPIRE（天朝上国 · 和平互利）

入向国际商路双方产出（埃及同款 API，纯 Modifier，无 Lua）：

| 受益方 | ModifierType | 产出 |
|--------|--------------|------|
| 对方出发城 | `…_TRADE_ROUTE_YIELD_TO_OTHERS` | +1 科、+1 文 |
| 本方到达城 | `…_TRADE_ROUTE_YIELD_FROM_OTHERS` | +4 金、+2 信仰 |
