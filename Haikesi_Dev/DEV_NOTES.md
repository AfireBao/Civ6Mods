...
- 新增权重 300 的「爱之法庭」，仅英格兰/法国埃莉诺、松迪亚塔·凯塔与克里斯缇娜可选；按城市区域为巨作提供额外产出。
- 持有「爱之法庭」时禁用安善的巨作科技值，且兼容保留先加载模组（包括 BBG）已有的安善条件。
- 新增罗伯特一世专属「化学极乐」（权重 300）：奢侈品额外覆盖 4 城宜居度；开心城市文化/信仰 +10%，欣喜若狂城市提高至 +20%。
- 新增旃陀罗笈多专属「政事论」（权重 300）：复刻 BBG 宗教建筑训练体系并补回旧版能力；神社 +1 移动/+2 战斗力，寺庙再 +1 视野/+2 战斗力且每城 +1 大将军点数，礼拜建筑再 +2 战斗力并提供陆军军团/集团军组建 25% 折扣。
- 新增武则天专属「罗织经」（权重 300）：立即追加 1 名免费间谍与 1 点间谍容量，进攻任务额外获得目标城市当回合 50% 科技/文化；保留信仰购买间谍，并独立复刻 BBG 的侦察单位 3 格陆地战斗单位 +3 战斗力光环。
- 新增林肯专属「解放黑奴」（权重 300）：工业区/水渠 +100% 生产力；工业区在官方 +2 基础上额外 +1 宜居度，合计 +3，并保留 +3 忠诚度；种植园在原有 -2 外额外 -3，合计 -5 忠诚度。额外忠诚度惩罚由海克斯通过 `COLLECTION_PLAYER_IMPROVEMENTS` 遍历持有者的种植园并逐座挂载，避免改良上下文无法响应运行时玩家 Property；已领取该海克斯的旧档通过版本 Property 只补挂一次新链路。工人可直接收获 `Improvement_ValidResources` 中登记为可建造种植园的奢侈品或加成资源，无需先建造种植园；通过隐藏代理资源复用原生收获结算，获得食物和等量金币、移除已有改良、消耗 1 次劳动力并生成当前最先进近战单位，随后直接启用官方 `ABILITY_LINCOLN_MELEE_UNITS`，自动跟随原版/BBG 的赠兵属性差异。
...
| 系列 | 成员 |
|------|------|
| 属性叠属性叠属性 | `NW_AI_STATS_1`…`6` |
| 回响施放 | `NW_AI_ECHO_*`（12；新增海军近战、海军远程、战斗机、轰炸机） |
| 混乱干扰 | `NW_AI_BARBARIAN_INVASION`、`NW_AI_LIGHTNING_STORM`、`NW_AI_RIVER_FLOOD` |
...
## NW_AI_BARBARIAN_INVASION（南蛮入侵 · 混乱干扰）

1. **必须开新档**：修改 `Haikesi_AI_Relics.sql` 后需重新开局。
2. **只启用 Dev 版**：工坊原版池更小，且与 Dev 同 Mod ID 易混用。
3. **重复性按效果区分**：`IsRepeatable=0` 的卡每个 AI 终身最多各 1 次。闪电风暴为一次性，避免再次抽中覆盖全局剩余回合；使者立足、征召上前为即时效果，允许重复获得。
4. **互斥逻辑**：房主随机、确定性补齐和超时回退中，每轮全场至多 1 个 AI 抽中混乱干扰类；ExtAI 明确提交的选择跳过该互斥。仅剩混乱干扰可选的 AI 会在非 ExtAI 流程中优先占用本轮名额。
...
选取与触发者关系最差的最多 3 名存活主要文明（**未接触 = 默认分 0**，等同中立）；收集其城市 3 环内可泛滥命名河，从下一回合起连续 3 回合对这些河 `ApplyEvent` 官方洪水（**70% 千年 / 30% 重大**）。无可用河则空放。与其他混乱干扰共享每轮互斥。实现：`Haikesi_RiverFlood_GamePlay.lua`。

## 和平互利系列（入向国际商路 · 埃及同款 API）

纯 Modifier，无 Lua。通往持卡文明城市的国际商路：

| RelicType | 名称 | 对方出发城 | 本方到达城 | 历史原型 |
|-----------|------|------------|------------|----------|
| `NW_AI_CELESTIAL_EMPIRE` | 天朝上国 | +1 科、+1 文 | +4 金、+2 信仰 | 唐·朝贡科文外溢 |
| `NW_AI_FERTILE_CRESCENT` | 两河粮仓 | +1 粮、+1 产 | +3 金、+1 粮 | 古代近东粮道 |
| `NW_AI_PAX_ROMANA` | 罗马和平 | +1 产、+1 金 | +3 金、+1 产 | 地中海秩序 |
| `NW_AI_SPICE_ROUTE` | 香料航路 | +1 粮、+1 金 | +4 金、+1 科 | 印度洋季风贸易 |
| `NW_AI_TRANS_SAHARAN` | 撒哈拉金路 | +1 信仰、+1 文 | +4 金、+1 粮 | 西非金盐与朝觐商路 |

## 商路增强

| RelicType | 名称 | 路线 | 容量 | 每条商路额外产出 |
|-----------|------|------|------|------------------|
| `NW_AI_TALENT_FLOW` | 人才流动 | 国际 | +1 | +2 科、+2 文、+2 产 |
| `NW_AI_FREE_TRADE` | 自由贸易 | 国际 | +1 | +8 金、+2 产 |
| `NW_AI_PILGRIMAGE_ROAD` | 朝圣之路 | 国际 | +1 | +4 信仰、+2 文 |
| `NW_AI_CLOSED_COUNTRY` | 闭关锁国 | 国内 | +1 | +4 金、+2 粮、+2 产 |
| `NW_AI_CLOSED_DOOR_WORKS` | 闭门造车 | 国内 | +1 | +3 科、+3 产 |
| `NW_AI_TEMPLE_ESTATES` | 香火田庄 | 国内 | +1 | +4 信仰、+2 粮 |
