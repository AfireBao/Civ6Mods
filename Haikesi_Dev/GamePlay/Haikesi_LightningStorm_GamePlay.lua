...
-- 选中当回合不起暴；从下一 TurnBegin 起连续 5 个游戏回合各造一轮。
-- 每回合场次 = floor(存活主要文明数 / 2)，至少 1。
...
-- 场次削弱：存活主要文明数的一半（向下取整），至少 1
local function StormBurstCount()
    local alive = CountAliveMajorCivs()
    if alive <= 0 then
        return 0
    end
    local half = math.floor(alive / 2)
    if half < 1 then
        return 1
    end
    return half
...
local function FireStormBurst(iPlayer, reason)
    ...
    print(string.format(
        '[Haikesi LightningStorm] start P%d storms=%d (alive=%d half) reason=%s turn=%s',
        iPlayer, count, alive, tostring(reason), tostring(Game.GetCurrentGameTurn())))
...
local function InitializeLightningStorm()
    ...
    print('[Haikesi LightningStorm] GamePlay ready (next-turn 5-burst, half majors)')
end
