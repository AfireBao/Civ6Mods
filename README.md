...
| 2026-08-03 | **弑杀蜂群生物质聚合优化**：不再为每 1 点限时食物创建一个市中心建筑。城市只保留一个代表当前总量的生物质档位建筑，独立击杀批次和到期时间改由城市 Property 聚合管理。单城达到 **+40 食物**上限后，会按击杀点距离把超出部分继续分配给下一座未满城市，避免收益被直接吞掉。 |
...
| 2026-07-31 | **世外桃源叠层调整**：基础 `世外桃源` 恢复为所有城市地块 **+1 魅力**，惊艳地块 **+1 生产力**不变；`世外桃源++` 不再提供额外魅力，改为完成法典后允许建造者种植树林，惊艳地块产出由 **+1 科技 / +1 文化**调整为 **+2 金币 / +1 信仰**。 |
