...
| 上传日期 | 描述 |
|----------|------|
| 2026-08-03 | **新增海克斯“虚空宗主”**：无前置、可重复、权重30000；每次获得后从10项随机城邦宗主能力中一次选择两项永久获得。选择面板按“城邦名称之力：城邦能力描述”展示，并过滤提供特色改良、已经选择过及与爱之法庭互斥的城邦能力。确认时会解开原版宗主国判定外壳，一次性挂载所选城邦的全部实际能力组件。 |
| 2026-08-03 | **归娟老虎机结算时序重构**：老爷归娟时仅完成单位转化、老虎机摇号与奖励快照，不再从 Gameplay 调用栈直接操作 UI 或提前发奖。UI 在下一更新帧通过玩家 Property 自行打开；玩家关闭界面后再经 `EXECUTE_SCRIPT` 按唯一编号结算南蛮、赠送单位或科文金信奖励。待结算结果支持读档恢复，重复关闭/重试会被幂等忽略，以规避奖励刷新与弹窗同栈造成的原生崩溃。 |
| 2026-08-03 | **死亡崇拜项目修复**：`异端献祭` 新增死亡崇拜专属的玩家级项目许可门，并继续以圣地隐藏建筑限制具体城市；选择 `米凯尔的祝福` 时会立即清理任何残留令牌，项目不再提前可见。项目完成结算改由原生完成 Modifier 写入城市计数器，再通过生产完成事件与回合开始双重消费，修复 `CityProjectCompleted` 未触发导致不扣人口、不生成丧尸的问题。若附近没有合法生成位置，会返还本次人口，避免只扣人口不出兵。 |
| 2026-08-03 | **弑杀蜂群生物质聚合优化**：不再为每 1 点限时食物创建一个市中心建筑。城市只保留一个代表当前总量的生物质档位建筑，独立击杀批次和到期时间改由城市 Property 聚合管理；旧存档的 `pin:expire` 会自动迁移。单城达到 **+40 食物**上限后，会按击杀点距离把超出部分继续分配给下一座未满城市，避免收益被直接吞掉。 |
| 2026-08-03 | **吸血鬼大公远程力调整**：远程力不再使用写死的 5 / 10 / 20 / 30 / 40，改为按科技阶段读取当前规则集中的投石兵、弓箭手、弩手、野战加农炮和机关枪远程力；兼容原版及 BBG 对这些单位的数值修改。 |
| 2026-08-03 | **丧尸基础战斗力优化**：丧尸改为与吸血鬼相同，基础战斗力随玩家最高近战战斗力变化；新增固有 Ability“腐肉脆骨”，使丧尸 **-12 战斗力**。丧尸本身不会计入玩家最高战斗力，避免递归抬升。 |
| 2026-08-03 | **独立总督模组实测修复**：补入阿玛妮城邦商路能力缺失的 UI→Gameplay 属性同步，派遣、就职、商路变化、读档与回合开始时均会重算，令通向阿玛妮所在城邦的己方城市正常获得 **+2 食物 / +2 生产力**；维克多“驻军司令”改用易卜拉欣同款友军领土战斗 Modifier，使城市领土内的己方单位进攻与防守均获得 **+5 战斗力**。 |
| 2026-07-31 | **刚果与波兰领袖专属海克斯「圣迹巡礼」**：权重 **300**，仅姆本巴·恩津加、恩津加·姆班德与雅德维加可选。领取时立即获得 **1 个遗物**；此后每发现一座自然奇观，再获得 **1 个遗物**，自然奇观触发复刻康缇宗主国能力的原生 Modifier。 |
...
| 2026-07-27 | **血色契约海克斯叠层**：`德古拉` 更名为 `血色契约：血裔`；新增 `血色契约：伯爵`（前置血裔，权重 **50**，吸血鬼 **+1 移动**、击杀回血、可建吸血鬼城堡）与 `血色契约：大公`（前置伯爵，权重 **25**，吸血鬼城堡快速移动并赠送吸血鬼大公）。大公复用吸血鬼模型，拥有 2 射程远程攻击（随科技依次对应投石兵、弓箭手、弩手、野战加农炮和机关枪）、**5%** 击杀转化、专属晋升树（法杖贴脸远程 **+7**）与透明底蝙蝠单位图标。 |
...
| 2026-07-24 | **玩家海克斯「弑杀蜂群」**（`WARFEEDRUNE`）：击杀敌军后，按距离向己方城市分配其战斗力 **5%** 的食物，持续 10 回合；单城达到 **+40 食物**上限后继续查询下一座未满城市。多次击杀的批次通过城市 Property 独立计时，并聚合为单个生物质总量建筑，配套虫群生物质图标、粮食飞字及城市面板剩余回合提示。 |
