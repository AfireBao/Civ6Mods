...
| 2026-07-23 | **玩家海克斯「盲盒地脉」**（`LANDLOTTERYRUNE`）：城市可工作地块按格独立重随机**地形层**粮/锤/金/信仰（地貌/资源不动；山脉跳过；原地形合计不足 2 点提到 2）。实现为玩家级抵消 `Terrain_YieldChanges` + Plot Property 加回；扩张吃地再开盲盒。**联机**：回合末待刷格按 `plotIndex` 排序消费同步 RNG，去掉 `math.random` 兜底。 |
