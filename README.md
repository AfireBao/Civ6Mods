...
| 2026-07-28 | **三位一体老虎机海克斯**：合并并替代 `掌上明猪` 与 `高翔导航`，新增 `三位一体`（权重 **100**，无前置）：首都获得 1 只 `娟`。`娟 / 翔 / 捞叶` 可依次隐退转换，新形态行动力清零；`翔` 与新单位 `捞叶` 均不可捕获，被敌方踩中时回城。`捞叶` 隐退后获得 1 只 `娟` 并进入老虎机界面：2 娟获得本时代或上一时代科技随机单位，3 娟获得下一时代科技随机单位（最后时代回退本时代）；2/3 叶分别获得当前回合科文金信收入的 **1/5 / 1**；2 翔在最新城市 4 环刷出 2 个当前时代蛮族近战单位，3 翔对最新城市触发一次南蛮入侵。 |
...
| 2026-07-27 | **领土控制线海克斯调整**：`高翔导航` 改为前置 `坚壁清野` 后进入刷新池。 |
...
| 2026-07-26 | **海克斯平衡性调整**：坚壁清野己方领土非队友 **-2 移动力**（射程 -1 不变）；利刃华尔兹敌境战斗力 **+3→+5**；将军金印大将军加力 **+100%**（额外 +5）；仇水连汛洪水 **5→3 回合**；铝翼坠毁赠送**三星**直升机；世外桃源魅力 **+2→+1**；精英政治每位伟人首都全产 **+1→+3**；裂变（仿生战士）首都初始赠送 **1→3**。另修种地仙人 / 生物质壳 artdef：成员改为原版 `Builder_Ancient01..04`，修复地图无模型。 |
