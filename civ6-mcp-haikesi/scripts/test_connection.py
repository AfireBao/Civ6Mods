...
async def main() -> None:
    ...
    # Step 4: Try reading game state from the GameCore_Tuner Lua state.
    if lua_states:
        gamecore_state = 0
        try:
            name_index = lua_states.index("GameCore_Tuner")
            if name_index > 0:
                gamecore_state = int(lua_states[name_index - 1])
        except (ValueError, TypeError):
            pass

        print(f"4. Querying game turn number in state {gamecore_state}...")
        result = await execute_lua(
            reader,
            writer,
            gamecore_state,
            "print(tostring(Game.GetCurrentGameTurn()))",
        )
        if result:
