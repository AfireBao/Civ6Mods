...
def build_civ_religion_beliefs_query(player_ids: list[int] | None = None) -> str:
    """Read pantheon + founded religion tenets for majors (InGame).

    If player_ids is None/empty, dumps all alive major civs.
    Emits FAITH| / FBELIEF| lines.
    """
    if player_ids:
        ids = sorted({int(i) for i in player_ids if int(i) >= 0})
        id_list = ", ".join(str(i) for i in ids)
        viewers_lua = f"local viewers = {{{id_list}}}"
        iterate_lua = """
...
local function dumpCiv(pid)
  local cfg = PlayerConfigurations[pid]
  local civName = cfg and safeLookup(cfg:GetCivilizationShortDescription()) or ("Player"..pid)
  local leaderName = cfg and safeLookup(cfg:GetLeaderName()) or ""
  local panType, panName = "NONE", ""
  local relType, relName = "NONE", ""
  pcall(function()
    local pr = Players[pid]:GetReligion()
    local pan = pr:GetPantheon()
  ...
  print("FAITH|" .. pid .. "|" .. civName .. "|" .. leaderName
      ...
      if religion.Founder == pid and religion.Beliefs then
        for _, beliefIdx in ipairs(religion.Beliefs) do
          ...
          if b and b.BeliefClassType ~= "BELIEF_CLASS_PANTHEON" then
            local desc = safeLookup(b.Description):gsub("\\n", " ")
            print("FBELIEF|" .. pid .. "|" .. tostring(b.BeliefClassType)
...
def parse_civ_religion_beliefs_response(lines: list[str]) -> list[CivReligionBeliefs]:
    """Parse FAITH|/FBELIEF| output from build_civ_religion_beliefs_query."""
    by_id: dict[int, CivReligionBeliefs] = {}
    for raw in lines:
        line = raw.strip()
        ...
        if line.startswith("FAITH|"):
            ...
            pan_name = p[5] if pan_type else None
            rel_type = p[6] if p[6] and p[6] != "NONE" else None
            rel_name = p[7] if rel_type else None
            by_id[pid] = CivReligionBeliefs(
                player_id=pid,
                civ_name=p[2],
                leader_name=p[3],
                pantheon_type=pan_type,
                pantheon_name=pan_name or None,
                religion_type=rel_type,
                religion_name=rel_name or None,
            )
        elif line.startswith("FBELIEF|"):
            p = line.split("|", 5)
            if len(p) < 6:
                continue
            pid = int(p[1])
