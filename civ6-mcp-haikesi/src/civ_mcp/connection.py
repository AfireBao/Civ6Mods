...
class GameConnection:
    ...
    def __init__(self, host: str = "127.0.0.1", port: int = 4318):
        ...
        self.haikesi_gameplay_index: int | None = None
    ...
    async def connect(self) -> None:
        ...
        self.haikesi_gameplay_index = None
        i = 0
        while i + 1 < len(raw_states):
            try:
                ...
                if name == "Haikesi_GamePlay_Script" and self.haikesi_gameplay_index is None:
                    self.haikesi_gameplay_index = idx
                i += 2
            except ValueError:
                i += 1

        log.info(
            "Discovered %d Lua states (GameCore=%s, InGame=%s, Haikesi=%s)",
            len(self.lua_states),
            self.gamecore_index,
            self.ingame_index,
            self.haikesi_gameplay_index,
        )
    ...
    async def _ensure_haikesi_state(self) -> None:
        """Ensure Haikesi_GamePlay_Script Lua state is available."""
        await self.ensure_connected()
        if self.haikesi_gameplay_index is None:
            log.info("Haikesi state not found, reconnecting to re-discover...")
            await self.reconnect()
        if self.haikesi_gameplay_index is None:
            raise ConnectionError(
                "Haikesi_GamePlay_Script state not found. "
                "Make sure Haikesi mod is enabled and a game is in progress."
            )

    async def execute_haikesi(self, lua_code: str, timeout: float = 5.0) -> list[str]:
        """Execute Lua in Haikesi_GamePlay_Script context (mod AI relic protocol)."""
        await self._ensure_haikesi_state()
        return await self._execute_and_collect(self.haikesi_gameplay_index, lua_code, timeout)
