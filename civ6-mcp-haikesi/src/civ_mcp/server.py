...
@mcp.tool(annotations={"readOnlyHint": True})
async def get_civ_religion_beliefs(
    ctx: Context, player_id: int | None = None
) -> str:
    """Get pantheon and religion tenets for major civs.

    Args:
        player_id: Optional single player id. If omitted, returns all alive majors.

    Shows each civ's pantheon belief and founded religion beliefs (Follower,
    Founder, Enhancer, Worship, etc.). Useful for AI/opponent religion intel.
    """
    gs = _get_game(ctx)

    async def _run():
        ids = [player_id] if player_id is not None else None
        rows = await gs.get_civ_religion_beliefs(ids)
        return nr.narrate_civ_religion_beliefs(rows)

    return await _logged(
        ctx,
        "get_civ_religion_beliefs",
        {"player_id": player_id},
        _run,
    )
...
@mcp.tool(annotations={"readOnlyHint": True})
async def get_haikesi_ai_request(ctx: Context) -> str:
    """Poll for a pending Haikesi mod AI relic decision request.

    Use when NW_HAIKESI_EXTERNAL_AI is enabled in the game. After a human
    player confirms a hex choice, the mod creates a pending request that
    must be resolved by submitting AI relic choices via submit_haikesi_ai_choices.

    Returns JSON:
      - status \"none\" if no pending request
      - status \"pending\" with request_id, human_relic,
        and per-AI options (up to 6 randomly drawn relics, like human card picks)

    Rules for the LLM:
      - Pick exactly one relic per AI from that AI's \"options\" list only
      - Chaos interference relics (barbarian / lightning / flood) may be chosen by
        multiple AIs in the same ExtAI round (no mutex); non-ExtAI random still mutexes
        in GamePlay
    ...
    async def _run() -> str:
        lines = await gs.conn.execute_haikesi(haikesi_lua.build_get_ai_request_lua())
        return haikesi_lua.format_ai_request_json(lines)

    return await _logged(ctx, "get_haikesi_ai_request", {}, _run)
...
async def submit_haikesi_ai_choices(
    ...
    Connect FireTuner / civ6-mcp on the **game host** only. Submit validates and
    stages the payload on the host; the in-game ExtAI UI bridge then broadcasts
    via EXECUTE_SCRIPT so all peers apply the same AI relics (MP PVE safe).
    ...
    Args:
        ...
        reasons: Map of AI player ID string to a short Chinese decision reason
                 ...
                 Provide one reason per AI in choices when using external LLM mode.

    Returns JSON with ok=true/false, staged=true when host-staged for broadcast,
    and a message from the mod script.
    """
    gs = _get_game(ctx)
    ...
    return await _logged(
        ctx,
        "submit_haikesi_ai_choices",
        {"request_id": request_id, "choices": choices, "reasons": reasons},
        _run,
    )
...
async def run_lua(ctx: Context, code: str, context: str = "gamecore") -> str:
    ...
    Args:
        ...
        context: "gamecore" (default) for read-only state queries.
                 ...
                 "haikesi" for Haikesi_GamePlay_Script mod protocol.
    ...
    Context differences:
      ...
      haikesi:  Haikesi mod GamePlay script (external AI relic get/submit).
