...
def narrate_civ_religion_beliefs(rows: list[lq.CivReligionBeliefs]) -> str:
    """Narrate pantheon + religion tenets for one or more civs."""
    if not rows:
        return "No major civilization religion data available."
    lines: list[str] = []
    for row in rows:
        header = f"P{row.player_id} {row.civ_name} ({row.leader_name})"
        lines.append(header)
        if row.pantheon_name:
            lines.append(
                f"  Pantheon: {row.pantheon_name} ({row.pantheon_type})"
            )
        else:
            lines.append("  Pantheon: none")
        if row.religion_name:
            lines.append(
                f"  Religion: {row.religion_name} ({row.religion_type})"
            )
        else:
            lines.append("  Religion: not founded")
        for b in row.beliefs:
            if b.belief_class == "BELIEF_CLASS_PANTHEON":
                continue  # already summarized above
            short = b.belief_class.replace("BELIEF_CLASS_", "").title()
            lines.append(f"  [{short}] {b.name} ({b.belief_type}): {b.description}")
        lines.append("")
    return "\n".join(lines).rstrip()
