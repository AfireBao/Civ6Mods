...
class GameState:
    ...
    ) -> list[lq.CivReligionBeliefs]:
        """Pantheon + founded religion tenets for one/more/all major civs."""
        lua = lq.build_civ_religion_beliefs_query(player_ids)
        lines = await self.conn.execute_write(lua)
        return lq.parse_civ_religion_beliefs_response(lines)
